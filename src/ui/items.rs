...
pub fn view(state: &State) -> Element<'_, Message> {
    ...
    if let Some(id) = state.viewing_item_id {
        if let Some(item) = state.items.iter().find(|i| i.id == id) {
            return detail_view(item, state.view_image.as_deref());
        }
    }
    list_view(state)
}
...
fn list_view(state: &State) -> Element<'_, Message> {
    ...
    let page_items = filtered.iter().skip(start).take(PAGE_SIZE);

    let mut list = column![].spacing(theme::SPACE_SM).padding(theme::SPACE_MD);
    for item in page_items {
        list = list.push(item_row(item, state.confirming_delete_id == Some(item.id)));
    }
    if total == 0 {
...
fn item_row(item: &Item, confirming_delete: bool) -> Element<'_, Message> {
    let mut name_col = column![text(&item.name).size(16)].spacing(4);
    if item.has_image {
        name_col = name_col.push(text("📷 has photo").size(11));
    }
    ...
    container(
        row![
            container(name_col).width(Length::FillPortion(3)),
...
fn form_view(form: &ItemForm) -> Element<'_, Message> {
...
fn detail_view<'a>(item: &'a Item, image_bytes: Option<&'a [u8]>) -> Element<'a, Message> {
